...
#impute NA values
# create a dataset similat to population test ie without targer
trainForImpute <- population_train_data[,-ncol(population_train_data)]

population_train_data <- DMwR::knnImputation(population_train_data,k=5)
colSums(is.na(population_train_data))

#imputing values from traindata
population_test_data <- DMwR::knnImputation(population_test_data,k=5,distData = trainForImpute)
colSums(is.na(population_test_data))
...
preds_validate <- ifelse(prob_validate > 0.28, "1", "0")
...
preds_test <- ifelse(prob_test > 0.28, "1", "0")
